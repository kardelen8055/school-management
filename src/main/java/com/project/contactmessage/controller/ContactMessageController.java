...
public class ContactMessageController {
   ...
   public ResponseMessage<ContactMessageResponse> saveContact(@Valid @RequestBody ContactMessageRequest contactMessageRequest) {
    ...
    public Page<ContactMessageResponse>searchBySubjectByPage(
...
// Not: *********************************** getByIdWithParam ***************************************
   ...
   @GetMapping("/page")
    ...
    public  ResponseMessage<Page<ContactMessage>>getAllByPage(
            @RequestParam(value ="page", defaultValue = "0") int page,
            @RequestParam(value = "size", defaultValue = "10") int size,
            @RequestParam(value = "sort", defaultValue = "dateTime") String sort,
            @RequestParam(value = "type", defaultValue = "desc") String type){
...
@RequestBody anotasyonu, HTTP isteği gövdesinde gönderilen veriyi otomatik
olarak uygun Java nesnesine dönüştürür.
 HTTP isteği gövdesindeki veriyi, bir metot parametresine bağlamak için kullanılır.
 Bu anotasyon, özellikle RESTful web servislerinde POST, PUT, PATCH gibi HTTP metodlarıyla gönderilen JSON, XML
 veya diğer veri formatlarını, Java nesnelerine dönüştürmek için kullanılır.
